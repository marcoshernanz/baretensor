 ...
 */
[[nodiscard]] int64_t normalize_dim(const int64_t dim, const int64_t rank) noexcept {
 ...
 */
std::vector<int64_t> normalize_permutation_checked(std::string_view operation_name,
  ...
  if (static_cast<int64_t>(dims.size()) != rank) {
    std::ostringstream oss;
    oss << operation_name << " failed for tensor with shape " << shape_to_string(shape)
        << ": expected " << rank << " dims but got " << dims.size() << ".";
    throw std::invalid_argument(oss.str());
  }
  ...
  for (size_t i = 0; i < dims.size(); ++i) {
    ...
    if (normalized < 0 || normalized >= rank) {
      std::ostringstream oss;
      oss << operation_name << " failed for tensor with shape " << shape_to_string(shape)
          << ": dims[" << i << "]=" << dim << " is out of range for rank " << rank << ".";
      throw std::invalid_argument(oss.str());
    }
    if (seen[static_cast<size_t>(normalized)]) {
      std::ostringstream oss;
      oss << operation_name << " failed for tensor with shape " << shape_to_string(shape)
          << ": dimension " << normalized << " appears more than once.";
      throw std::invalid_argument(oss.str());
    }
