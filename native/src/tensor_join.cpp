 ...
 * Purpose: Implements tensor concatenation and its autograd node.
 */
...
[[nodiscard]] std::string cat_shapes_to_string(const std::vector<bt::Tensor> &tensors) {
...
[[nodiscard]] bool should_record_cat(const std::vector<bt::Tensor> &tensors) {
...
Tensor cat(const std::vector<Tensor> &tensors, const int64_t dim) {
  ...
  for (const Tensor &tensor : tensors) {
    bt::detail::validate_copy_metadata(tensor, "cat");
  }

  const bool should_record = should_record_cat(tensors);
