...
class DTypeTests(unittest.TestCase):
    def test_bool_structural_ops_preserve_dtype(self) -> None:
        source = bt.tensor(np.asarray([[True, False], [False, True]], dtype=np.bool_))

        viewed = source.view([4])
        transposed = source.transpose(0, 1)
        contiguous = transposed.contiguous()

        self.assertEqual(viewed.dtype, bt.bool)
        self.assertEqual(transposed.dtype, bt.bool)
        self.assertEqual(contiguous.dtype, bt.bool)
        self.assertEqual(to_numpy(contiguous).dtype, np.bool_)
        np.testing.assert_array_equal(
            to_numpy(contiguous),
            np.asarray([[True, False], [False, True]], dtype=np.bool_).T,
        )
    ...
    def test_numpy_and_item_preserve_bool(self) -> None:
        scalar = bt.tensor(True)

        array = to_numpy(scalar)

        self.assertEqual(array.dtype, np.bool_)
        self.assertEqual(cast(bool, scalar.item()), True)
    ...
    def test_bool_math_ops_reject(self) -> None:
        tensor = bt.tensor([[True, False], [False, True]], dtype=bt.bool)
    ...
    def test_bool_to_handles_non_contiguous_views(self) -> None:
        tensor = bt.tensor(np.asarray([[1, 0, 1], [0, 1, 0]], dtype=np.int64)).transpose(0, 1)

        converted = tensor.to(bt.bool)

        self.assertEqual(converted.dtype, bt.bool)
        np.testing.assert_array_equal(
            to_numpy(converted),
            np.asarray([[True, False], [False, True], [True, False]], dtype=np.bool_),
        )
